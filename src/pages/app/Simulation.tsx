  ...
  return (
    <div className="space-y-6">
      ...
      {(lastResult || comparisonResult) && <div className="space-y-6">
        ...
        {comparisonResult && <Card variant="strong" className="border-l-4 border-flood-success"><CardHeader><CardTitle className="flex items-center gap-2"><TrendingUp className="w-5 h-5 text-flood-success" />Intervention Impact Analysis</CardTitle></CardHeader><CardContent className="space-y-6"><div className="grid grid-cols-2 md:grid-cols-4 gap-4">{[ { label: 'Affected Roads', before: comparisonResult.before.affectedRoads, after: comparisonResult.after.affectedRoads, unit: '', good: 'down' }, { label: 'High Risk Roads', before: comparisonResult.before.highRiskRoads, after: comparisonResult.after.highRiskRoads, unit: '', good: 'down' }, { label: 'Critical Infra', before: comparisonResult.before.criticalInfrastructure, after: comparisonResult.after.criticalInfrastructure, unit: '', good: 'down' }, { label: 'Drainage Stress', before: comparisonResult.before.drainageStress, after: comparisonResult.after.drainageStress, unit: '%', good: 'down' }, { label: 'Traffic Disruption', before: comparisonResult.before.trafficDisruption, after: comparisonResult.after.trafficDisruption, unit: '', good: 'down' }, { label: 'Est. Damage', before: comparisonResult.before.estimatedDamage, after: comparisonResult.after.estimatedDamage, unit: '', good: 'down', format: formatCurrency } ].map((metric, i) => (<Card key={i} variant="hover" className="text-center p-4"><p className="text-sm text-flood-muted mb-2">{metric.label}</p><div className="flex items-center justify-center gap-3"><div className="text-2xl font-bold text-flood-muted">{metric.format ? metric.format(metric.before) : metric.before}{metric.unit}</div><ArrowRight className="w-5 h-5 text-flood-primary" /><div className="text-2xl font-bold" style={{ color: metric.after < metric.before ? '#22c55e' : metric.after > metric.before ? '#ef4444' : '#06b6d4' }}>{metric.format ? metric.format(metric.after) : metric.after}{metric.unit}</div></div><div className="mt-2 text-sm" style={{ color: metric.after < metric.before ? '#22c55e' : metric.after > metric.before ? '#ef4444' : '#06b6d4' }}>{metric.after < metric.before ? `${Math.round(((metric.before - metric.after) / metric.before) * 100)}% reduction` : metric.after > metric.before ? `${Math.round(((metric.after - metric.before) / metric.before) * 100)}% increase` : 'No change'}</div></Card>))}</div><div className="p-4 bg-flood-success/10 rounded-lg border border-flood-success/30"><h4 className="font-medium text-flood-success mb-2 flex items-center gap-2"><CheckCircle className="w-5 h-5" />Key Finding</h4><p className="text-sm text-flood-text">Clearing <strong>Drain D-182</strong> reduces affected roads by <strong>{Math.round(((comparisonResult.before.affectedRoads - comparisonResult.after.affectedRoads) / comparisonResult.before.affectedRoads) * 100)}%</strong> and estimated damage by <strong>{formatCurrency(comparisonResult.before.estimatedDamage - comparisonResult.after.estimatedDamage)}</strong>. Recommended as Priority 1 intervention.</p></div></CardContent></Card> }
