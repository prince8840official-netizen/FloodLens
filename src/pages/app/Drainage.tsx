  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid lg:grid-cols-[2fr_1fr] gap-6">
        ...
        <div className="space-y-6">
          <Card variant="strong"><div className="p-4 border-b border-flood-border"><h2 className="font-semibold text-flood-text">Drain Status Overview</h2></div><CardContent className="p-4 space-y-3">{filteredDrains.map(drain => (<div key={drain.id} className="flex items-center justify-between p-3 bg-flood-bg rounded-lg hover:bg-flood-card/50 cursor-pointer" onClick={() => onDrainClick(drain)}><div className="flex items-center gap-3"><span className="font-mono text-xs text-flood-primary">{drain.id}</span><span className="text-sm font-medium">{drain.name}</span></div><div className="flex items-center gap-2"><StatusBadge status={drain.status as any} size="sm" /><span className="font-mono text-sm">{drain.capacity}%</span></div></div>))}</CardContent></Card>
      ...
      <Card variant="strong"><div className="p-4 border-b border-flood-border"><h2 className="font-semibold text-flood-text">Drainage Network Table</h2></div><CardContent className="p-0"><DataTable columns={[{ key: 'id', header: 'ID', render: (row: Drain) => <Link to={`/drainage/${row.id}`} className="font-mono text-xs text-flood-primary hover:underline">{row.id}</Link> }, { key: 'name', header: 'Name' }, { key: 'type', header: 'Type', render: (row: Drain) => <Badge variant="default" size="sm" className="capitalize">{row.type.replace('-', ' ')}</Badge> }, { key: 'capacity', header: 'Capacity %', render: (row: Drain) => <span className="font-mono" style={{ color: row.capacity > 70 ? '#ef4444' : row.capacity > 50 ? '#f97316' : '#06b6d4' }}>{row.capacity}%</span> }, { key: 'blockageProbability', header: 'Blockage %', render: (row: Drain) => <span className="font-mono" style={{ color: row.blockageProbability > 70 ? '#ef4444' : row.blockageProbability > 40 ? '#f97316' : '#22c55e' }}>{row.blockageProbability}%</span> }, { key: 'flowRate', header: 'Flow (L/s)', render: (row: Drain) => <span className="font-mono">{row.flowRate}</span> }, { key: 'depth', header: 'Depth (m)', render: (row: Drain) => <span className="font-mono">{row.depth}</span> }, { key: 'status', header: 'Status', render: (row: Drain) => <StatusBadge status={row.status as any} size="sm" /> }, { key: 'priority', header: 'Priority', render: (row: Drain) => <Badge variant={row.priority === 'P1' ? 'critical' : row.priority === 'P2' ? 'high' : 'moderate'} size="sm">{row.priority}</Badge> }, { key: 'lastMaintenance', header: 'Last Maint.', render: (row: Drain) => <span className="font-mono">{row.lastMaintenance}d ago</span> }, { key: 'historicalIncidents', header: 'Incidents', render: (row: Drain) => <span className="font-mono">{row.historicalIncidents}</span> }, { key: 'connectedRoads', header: 'Roads', render: (row: Drain) => <span className="font-mono text-xs">{row.connectedRoads.join(', ')}</span> }]} data={drains} keyExtractor={row => row.id} /></CardContent></Card>
    </div>
  );
}